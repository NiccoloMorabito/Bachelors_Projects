...
void OrazioWrapper::getVelocities(float &tv, float &rv) {
  ...
  OrazioClient_get(_client, (PacketHeader*)&drive_control);
  tv = drive_control.translational_velocity;
  rv = drive_control.rotational_velocity;
  //
}
...
void OrazioWrapper::getSonar(bool &is_new, int &range, int sonar_num) {
  ...
  if (currentEpoch() != sonar_status.header.seq) {
    is_new = false;
  }
  else {
